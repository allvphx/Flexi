...
func (stmt *TPCStmt) Init() {
	if configs.LocalTest {
		stmt.coordinator, stmt.participants = coordinator.TPCCTestKit()
	} else {
...
//	slRead := coordinator.NewTX(slTID, parts, c.coordinator.Manager)
//	for _, v := range order.Items {
//		slRead.AddRead("STOCK", configs.OuAddress[v.Warehouse], utils.TransTableItem(v.Warehouse, v.Item))
//	}
//	val, ok := c.coordinator.Manager.PreRead(slRead)
//	if !ok {
//		configs.TPrintf("Failed for Warehouse read")
//		return false
//	}
//	slWrite := coordinator.NewTX(slTID, parts, c.coordinator.Manager)
//	for i, v := range order.Items {
//		key := utils.TransTableItem(v.Warehouse, v.Item)
...
//	}
//	ok = c.coordinator.Manager.SubmitTxn(slWrite, c.protocol, nil)
//	return ok
//}
//
...
//	osRead := coordinator.NewTX(osTID, parts, c.coordinator.Manager)
//	for _, v := range order.Items {
//		key := utils.TransTableItem(v.Warehouse, v.Item)
//		osRead.AddRead("ORDER", configs.OuAddress[v.Warehouse], key)
//	}
//	_, ok := c.coordinator.Manager.PreRead(osRead)
//	osWrite := coordinator.NewTX(osTID, parts, c.coordinator.Manager)
//	ok = ok && c.coordinator.Manager.SubmitTxn(osWrite, c.protocol, nil)
//	return ok
//}
//
