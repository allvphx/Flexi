...
func Test3PCPreWrite(t *testing.T) {
	...
	res := ca.Manager.ThreePCSubmit(txn, nil)
	configs.Assert(res, "The 3PC Failed")
	configs.JPrint("3PC commit succeed")
...
func Test3PCSerialOrder(t *testing.T) {
	makeLocal()
	defer recLocal()
	ca, co := TestKit()
	...
	res := ca.Manager.ThreePCSubmit(w1, nil)
	participant.CheckVal(t, co[0].Manager, []string{"1", "2", "3", "4", "5"})
	participant.CheckVal(t, co[1].Manager, []string{"2", "3", "4", "5", "6"})
	participant.CheckVal(t, co[2].Manager, []string{"3", "4", "5", "6", "7"})
	res = res && ca.Manager.ThreePCSubmit(w2, nil)
	configs.Assert(res, "The 3PC Failed")
	participant.CheckVal(t, co[0].Manager, []string{"1", "2", "3", "4", "5"})
	participant.CheckVal(t, co[1].Manager, []string{"2", "3", "4", "5", "6"})
...
func Test3PCConcurrentExecution(t *testing.T) {
	makeLocal()
	defer recLocal()
	ca, co := TestKit()
	w1 := NewTX(1, address[:2], ca.Manager)
	...
	go func() {
		res := false
		for !res {
			res = ca.Manager.ThreePCSubmit(w2, nil)
			w2.TxnID += 2
		}
		ch <- res
	}()
